...
export function EnhancedAvailabilityCard({ availability, userId, onEditRequest }: EnhancedAvailabilityCardProps) {
  ...
  return (
    <Card className={`shadow-card transition-all duration-200 ${isInvitable ? 'ring-2 ring-primary/20 bg-gradient-to-r from-primary/5 to-transparent' : ''}`}>
      <CardContent className="p-4">
        <div className="flex items-start justify-between mb-3">
          ...
          <Button
            ...
            onClick={() => onEditRequest?.(availability)}
          >
          ...
          <div className="mb-3 p-3 rounded-lg bg-primary/10 border border-primary/20">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-sm font-medium text-primary">
                Altri possono invitarti
              </span>
            </div>
            <p className="text-xs text-muted-foreground">
              Le persone possono vedere che sei disponibile e invitarti a eventi in questo orario
            </p>
          </div>
        )}
