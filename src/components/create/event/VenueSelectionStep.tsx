...
export default function VenueSelectionStep({ data, onChange, onNext }: VenueSelectionStepProps) {
  ...
  const handleSearch = (query: string) => {
    ...
    const filtered = mockVenues.filter(venue =>
      ...
      venue.amenities.some(amenity => amenity.toLowerCase().includes(query.toLowerCase()))
    );
    setFilteredVenues(filtered);
  };

  const toggleVenueSelection = (venueId: string) => {
    const selectedVenues = data.selectedVenues.includes(venueId)
      ? data.selectedVenues.filter((id: string) => id !== venueId)
      : [...data.selectedVenues, venueId];
    
    onChange({ ...data, selectedVenues });
  };
    ...
    <div className="space-y-6">
      ...
      <div className="relative">
        ...
        <Input
          ...
          placeholder="Cerca location per nome, città o servizi..."
          className="pl-10"
        />
      </div>

      {/* Selected venues */}
      {data.selectedVenues.length > 0 && (
        <div>
          <h4 className="font-medium mb-3">Location selezionate ({data.selectedVenues.length})</h4>
          <div className="grid grid-cols-1 gap-3">
            {getSelectedVenues().map((venue) => (
              <Card key={venue.id} className="border-primary/50">
      ...
      <div>
        ...
        <div className="grid grid-cols-1 gap-3">
          {filteredVenues
            .filter(venue => !data.selectedVenues.includes(venue.id))
            .map((venue) => (
      ...
      {filteredVenues.filter(venue => !data.selectedVenues.includes(venue.id)).length === 0 && (
