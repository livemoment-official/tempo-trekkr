      ...
      <div className="flex items-center justify-between">
        ...
        {validation.overall.nextIncompleteStep && (
          <div className="flex items-center gap-1 text-xs text-muted-foreground">
            ...
            <span>{steps[validation.overall.nextIncompleteStep - 1]?.title}</span>
          </div>
        )}
      </div>
      ...
      {validation.steps.details.errors.length > 0 && currentStep === 1 && (
        <div className="bg-destructive/10 border border-destructive/20 rounded-lg p-2">
          ...
          <ul className="text-xs text-destructive/80 space-y-0.5">
            {validation.steps.details.errors.slice(0, 3).map((error, index) => (
              <li key={index}>• {error}</li>
            ))}
            {validation.steps.details.errors.length > 3 && (
              <li>• ... e altri {validation.steps.details.errors.length - 3}</li>
            )}
          </ul>
        </div>
      )}
