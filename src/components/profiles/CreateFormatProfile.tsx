...
const CATEGORIES = ["Musica Live", "DJ Set", "Teatro", "Arte & Cultura", "Sport & Wellness", "Food & Drink", "Networking", "Workshop", "Festival", "Altro"];
const ARTIST_CATEGORIES = ["Musicista", "DJ", "Band", "Performer", "Attore", "Artista Visivo", "Ballerino", "Altro"];
const STAFF_ROLES = ["Organizzatore", "Direttore Artistico", "Pubbliche Relazioni & Magnet", "Presentatore/Moderatore", "Maestro/Animatore", "Tecnico Audio/Video", "Videomaker/Fotografo", "Content Creator/Influencer", "Gestore Logistica", "Coordinatore Sicurezza"];
const LOCATION_TYPES = ["Club/Discoteca", "Bar/Pub", "Ristorante", "Teatro", "Sala Concerti", "Spazio Polivalente", "Location Outdoor", "Galleria d'Arte", "Centro Culturale", "Altro"];
export function CreateFormatProfile({
  onSubmit,
  onCancel
}: CreateFormatProfileProps) {
          ...
          <CardContent className="p-6">
            ...
            {currentStep === 1 && <div className="space-y-6">
                ...
                <div>
                  ...
                  {representativeImage ? <div className="relative w-full h-48 rounded-lg overflow-hidden">
                      <img src={representativeImage} alt="Rappresentativa" className="w-full h-full object-cover" />
                      <Button type="button" variant="destructive" size="icon" className="absolute top-2 right-2" onClick={() => setValue("representative_image", "")}>
                        <X className="h-4 w-4" />
                      </Button>
                    </div> : <label className="flex flex-col items-center justify-center w-full h-48 border-2 border-dashed rounded-lg cursor-pointer hover:bg-muted/50">
                      <Upload className="h-12 w-12 text-muted-foreground mb-2" />
                      <span className="text-sm text-muted-foreground">Carica immagine</span>
                      <input type="file" className="hidden" accept="image/*" onChange={e => e.target.files?.[0] && handleImageUpload(e.target.files[0], 'representative_image')} />
                    </label>}
                ...
                <div>
                  ...
                  <div className="grid grid-cols-3 gap-4 mb-4">
                    {supportGallery.map((url, index) => <div key={index} className="relative aspect-square rounded-lg overflow-hidden">
                ...
                <div>
                  ...
                  {logoUrl ? <div className="relative w-32 h-32 rounded-lg overflow-hidden">
                      <img src={logoUrl} alt="Logo" className="w-full h-full object-contain bg-muted" />
                      <Button type="button" variant="destructive" size="icon" className="absolute top-1 right-1 h-6 w-6" onClick={() => setValue("logo_url", "")}>
                        <X className="h-3 w-3" />
                      </Button>
                    </div> : <label className="flex flex-col items-center justify-center w-32 h-32 border-2 border-dashed rounded-lg cursor-pointer hover:bg-muted/50">
                      <Upload className="h-8 w-8 text-muted-foreground mb-2" />
                      <span className="text-xs text-muted-foreground">Logo</span>
                      <input type="file" className="hidden" accept="image/*" onChange={e => e.target.files?.[0] && handleImageUpload(e.target.files[0], 'logo_url')} />
                    </label>}
                </div>

                <div>
                  <Label htmlFor="description">Descrizione *</Label>
                  <Textarea id="description" {...register("description")} placeholder="Descrivi il tuo format..." rows={5} />
            ...
            {currentStep === 2 && <div className="space-y-6">
                ...
                <div>
                  ...
                  <div className="space-y-2 mb-2">
                    {activities.map((activity, index) => <div key={index} className="flex items-center gap-2">
                  ...
                  <div className="flex gap-2">
                    <Input value={newActivity} onChange={e => setNewActivity(e.target.value)} placeholder="Aggiungi attività" onKeyPress={e => e.key === 'Enter' && (e.preventDefault(), addActivity())} />
                ...
                <div>
                  ...
                  <div className="space-y-2 mb-2">
                    {materials.map((material, index) => <div key={index} className="flex items-center gap-2">
                  ...
                  <div className="flex gap-2">
                    <Input value={newMaterial} onChange={e => setNewMaterial(e.target.value)} placeholder="Aggiungi materiale" onKeyPress={e => e.key === 'Enter' && (e.preventDefault(), addMaterial())} />
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <Input id="avg_participants" type="number" {...register("avg_participants", {
                  valueAsNumber: true
                })} placeholder="50" />
                  ...
                  <div>
                    ...
                    <Input id="avg_cost_per_participant" type="number" {...register("avg_cost_per_participant", {
                  valueAsNumber: true
                })} placeholder="25" />
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-2 mt-2">
                    {ARTIST_CATEGORIES.map(category => <div key={category} className="flex items-center space-x-2">
                        <Checkbox id={`artist-${category}`} checked={selectedArtistCategories.includes(category)} onCheckedChange={() => toggleArtistCategory(category)} />
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-2 mt-2">
                    {STAFF_ROLES.map(role => <div key={role} className="flex items-center space-x-2">
                        <Checkbox id={`staff-${role}`} checked={selectedStaffRoles.includes(role)} onCheckedChange={() => toggleStaffRole(role)} />
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-2 mt-2">
                    {LOCATION_TYPES.map(type => <div key={type} className="flex items-center space-x-2">
                        <Checkbox id={`location-${type}`} checked={selectedLocationTypes.includes(type)} onCheckedChange={() => toggleLocationType(type)} />
                ...
                <div>
                  ...
                  <Input id="recommended_days" {...register("recommended_days")} placeholder="Es: Venerdì e Sabato sera" />
                ...
                <div>
                  ...
                  <div className="space-y-2 mb-2">
                    {eventTimings.map((timing, index) => <div key={index} className="flex items-center gap-2">
                  ...
                  <div className="flex gap-2">
                    <Input value={newTiming} onChange={e => setNewTiming(e.target.value)} placeholder="Aggiungi tempistica" onKeyPress={e => e.key === 'Enter' && (e.preventDefault(), addTiming())} />
            ...
            {currentStep === 3 && <div className="space-y-6">
                ...
                <div>
                  ...
                  {founderPhoto ? <div className="relative w-32 h-32 rounded-full overflow-hidden">
                      <img src={founderPhoto} alt="Founder" className="w-full h-full object-cover" />
                      <Button type="button" variant="destructive" size="icon" className="absolute top-1 right-1 h-6 w-6" onClick={() => setValue("founder_photo", "")}>
                        <X className="h-3 w-3" />
                      </Button>
                    </div> : <label className="flex flex-col items-center justify-center w-32 h-32 border-2 border-dashed rounded-full cursor-pointer hover:bg-muted/50">
                      <Upload className="h-8 w-8 text-muted-foreground mb-2" />
                      <span className="text-xs text-muted-foreground">Foto</span>
                      <input type="file" className="hidden" accept="image/*" onChange={e => e.target.files?.[0] && handleImageUpload(e.target.files[0], 'founder_photo')} />
                    </label>}
                ...
                <div>
                  ...
                  <Textarea id="founder_bio" {...register("founder_bio")} placeholder="Racconta chi sei e perché hai creato questo format..." rows={5} />
                ...
                <div>
                  ...
                  <Input id="founder_email" type="email" {...register("founder_email")} placeholder="email@esempio.com" />
                ...
                <div>
                  ...
                  <Input id="founder_phone" type="tel" {...register("founder_phone")} placeholder="+39 333 1234567" />
                  {errors.founder_phone && <p className="text-sm text-destructive mt-1">{errors.founder_phone.message}</p>}
                </div>

                <div className="flex items-start space-x-2">
                  <Checkbox id="privacy" onCheckedChange={checked => setValue("privacy_accepted", checked as boolean)} />
