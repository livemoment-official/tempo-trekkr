      ...
      <CardContent className="p-4">
        <div className="flex items-center justify-between mb-4">
          ...
          <label htmlFor="preview-photo-upload">
            <Button type="button" variant="outline" size="sm" asChild disabled={isUploading || previewPhotos.length >= 6}>
        ...
        {previewPhotos.length > 0 ? <div className="grid grid-cols-3 gap-2">
            {previewPhotos.slice(0, 6).map((photo, index) => <div key={index} className="relative group aspect-square">
                <img src={photo} alt={`Preview ${index + 1}`} className="w-full h-full object-cover rounded-lg" />
                <button onClick={() => removePhoto(photo)} className="absolute top-1 right-1 p-1 bg-red-500 text-white rounded-full opacity-0 group-hover:opacity-100 transition-opacity">
                  <X className="h-3 w-3" />
                </button>
              </div>)}
        ...
        {previewPhotos.length >= 6 && <div className="mt-3 text-xs text-muted-foreground text-center">
