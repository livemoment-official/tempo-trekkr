...
export function GroupRulesBanner({
  onDismiss
}: GroupRulesBannerProps) {
  ...
  return <>
      <div className="relative overflow-hidden rounded-lg border border-primary/20 bg-gradient-to-br from-yellow-50 via-orange-50 to-yellow-50 dark:from-yellow-950/20 dark:via-orange-950/20 dark:to-yellow-950/20 shadow-lg transition-all duration-300 hover:shadow-xl">
        {/* Header */}
        <button onClick={() => setIsExpanded(!isExpanded)} className="w-full flex items-center justify-between p-4 gap-3 hover:bg-black/5 dark:hover:bg-white/5 transition-colors">
          ...
          <div className="flex items-center gap-2">
            {onDismiss && <Button variant="ghost" size="icon" onClick={e => {
            e.stopPropagation();
            onDismiss();
        ...
        {/* Content */}
        {isExpanded && <div className="px-4 pb-4 animate-fade-in">
            ...
            <Button onClick={handleCreateGroup} className="w-full bg-primary hover:bg-primary/90 text-primary-foreground font-semibold shadow-md hover:shadow-lg transition-all">
      ...
      <AuthModal open={showAuthModal} onOpenChange={setShowAuthModal} title="Accedi per creare un gruppo" description="Devi effettuare il login per poter creare un gruppo" />
    </>;
}