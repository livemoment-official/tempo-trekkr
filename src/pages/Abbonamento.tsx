...
export default function Abbonamento() {
  ...
  return <div className="min-h-screen bg-gradient-to-b from-background via-background to-primary/5">
      ...
      <StandardHeader title="Abbonamento" onBack={handleBack} rightActions={<Button variant="ghost" size="icon" className="text-green-600 hover:text-green-700 hover:bg-green-50" onClick={() => window.open('https://wa.me/393334567890', '_blank')}>
      ...
      <div className="container mx-auto px-4 py-3 md:py-4 pb-[calc(72px+env(safe-area-inset-bottom))] md:pb-[calc(80px+env(safe-area-inset-bottom))] max-w-2xl">
        ...
        <div className="p-1 bg-muted/30 rounded-full mb-4 flex border border-border/50">
          <button onClick={() => setActiveTab('pro')} className={`flex-1 py-2.5 px-5 rounded-full text-xs font-medium transition-all duration-200 flex items-center justify-center gap-2 ${activeTab === 'pro' ? 'bg-gradient-to-r from-orange-400 to-orange-500 text-black shadow-md' : 'text-muted-foreground hover:text-foreground'}`}>
            <Crown className="h-4 w-4" />
            <span>Pro</span>
          </button>
          <button onClick={() => setActiveTab('business')} className={`flex-1 py-2.5 px-5 rounded-full text-xs font-medium transition-all duration-200 flex items-center justify-center gap-2 ${activeTab === 'business' ? 'bg-gradient-to-r from-orange-400 to-orange-500 text-black shadow-md' : 'text-muted-foreground hover:text-foreground'}`}>
        ...
        <div className="space-y-3 md:space-y-4">
          {activeTab === 'pro' ? <>
              ...
              <div className="space-y-3">
                {proFeatures.map((feature, index) => <Card key={index} className="border-0 bg-orange-50/40 hover:bg-orange-50/60 transition-all duration-200">
                ...
                <div className="grid grid-cols-3 gap-3">
                  {Object.entries(pricingPlans).map(([key, plan]) => <button key={key} onClick={() => setSelectedPlan(key as '1' | '3' | '6')} className={`relative p-4 rounded-3xl border-2 transition-all duration-200 ${selectedPlan === key ? 'border-primary bg-primary text-white shadow-xl shadow-primary/30' : 'border-border bg-muted/30 hover:border-primary/50'}`}>
                      ...
                      <div className="text-center">
                        <div className={`text-5xl font-bold mb-1 ${selectedPlan === key ? 'text-white' : 'text-foreground'}`}>
                          {plan.months}
                        </div>
                        <div className={`text-xs ${selectedPlan === key ? 'text-white/80' : 'text-muted-foreground'}`}>
                          {plan.months === '1' ? 'mese' : 'mesi'}
                        </div>
                        
                        {plan.saving && <div className={`mt-2 text-[10px] font-medium ${selectedPlan === key ? 'text-white' : 'text-green-600'}`}>
                            Risparmio
                          </div>}
                      </div>
                    </button>)}
                ...
                <Card className="border-0 bg-orange-50/40">
                  <CardContent className="p-5">
                    <div className="text-center space-y-2">
                      ...
                      {pricingPlans[selectedPlan].saving && <div className="inline-flex items-center gap-1.5 text-sm font-semibold text-green-600 bg-green-50 px-4 py-2 rounded-full">
              ...
              {/* Business Features */}
              <div className="space-y-3">
                {businessFeatures.map((feature, index) => <Card key={index} className="border-0 bg-orange-50/40 hover:bg-orange-50/60 transition-all duration-200">
                    <CardContent className="flex items-start gap-4 p-4">
                      <div className="flex-shrink-0 text-2xl">
                        {feature.icon}
                      </div>
