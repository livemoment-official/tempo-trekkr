...
export default function Abbonamento() {
  ...
  const pricingPlans = {
    '1': { months: '1', price: '9,99€', total: '9,99€', saving: '' },
    '3': { months: '3', price: '6,99€', total: '19,99€', saving: 'Risparmi 9,98€' },
    '12': { months: '12', price: '5,00€', total: '59,99€', saving: 'Risparmi 59,89€' }
  };
            ...
            <>
              ...
              <div className="space-y-4">
                {proFeatures.map((feature, index) => (
                  <div key={index} className={`flex gap-4 p-4 rounded-xl ${feature.bgColor}`}>
                  ...
                  {Object.entries(pricingPlans).map(([key, plan]) => (
                    ...
                    >
                      <div className="text-center">
                        ...
                        {plan.saving && (
                          <div className="text-xs text-green-600 font-medium mt-1">{plan.saving}</div>
                        )}
                      </div>
                    </button>
                  ))}
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-3">
                  {accountTypes.map((account, index) => (
                    <button
                      key={index}
                      onClick={() => setSelectedBusinessType(account.title.toLowerCase().replace('account ', ''))}
                      className={`w-full flex items-center justify-between p-4 rounded-xl border-2 transition-all ${
                        selectedBusinessType === account.title.toLowerCase().replace('account ', '')
                          ? 'border-orange-500 bg-orange-50'
